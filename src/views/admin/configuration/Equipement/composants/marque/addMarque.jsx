...
const Addmarque = ({ handleClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const marque = { name }
        const response = await fetch(`${backendUrl}/api/marque/add`, {
            method: 'POST',
        ...
        if (response.ok) {
            setEmptyFields([])
            setName('')
            setSuccessMessage("marque ajouté avec succès");
            handleClose()
        }
    }
    ...
    return (
        <div className="fixed top-0 left-0 z-10 flex h-full w-full items-center justify-center">
            <div className="rounded-md bg-white p-8 border-2 shadow-lg border-tunisys-100">
                <h2 className="mb-4 text-xl font-semibold text-center text-tunisys-100">Ajouter Marque</h2>
