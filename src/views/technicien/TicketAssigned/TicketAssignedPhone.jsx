...
const TicketAssigned = () => {
  ...
  useEffect(() => {
    fetch(`${backendUrl}/api/ticketht/assigned/phone`, {
      method: 'GET',
      headers: {
  ...
  const handleAcceptTicket = async (ticketId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`${backendUrl}/api/ticket/accepted/${ticketId}`, {
            method: "PUT",
            headers: {
  ...
  const handleStartTicket = async (ticketId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`${backendUrl}/api/ticket/started/${ticketId}`, {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
            },
                  ...
                  phoneticketData
                    ...
                    .map((phoneticket, index) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4"> {phoneticket.created_at ? format(new Date(phoneticket.created_at), 'yyyy/MM/dd ') : 'Not yet'}</td>
